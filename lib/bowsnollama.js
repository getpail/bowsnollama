...
async function handleQuery(data, ws, instance) {
  try {
    ...
    if(typeof message.data !== 'string' || message.data.trim() === '') {
      console.log('handleQuery error: query data is missing or not a string');
      return;
    }

    let ask = { role: "user", content: message.data };
    let request = { model: instance.model, messages: instance.chatHistory,
                    stream: true };

    // Append the user message to chat history
    instance.chatHistory.push(ask);

    // Abort any previous call, if needed
    instance.ollama.abort();

    // Make the chat request and await the response
    const response = await instance.ollama.chat(request);
    for await (const part of response) {
      if(part.message) {
        // Append assistant message to history
        instance.chatHistory.push({ role: "assistant",
                                    content: part.message.content });
      }

      // Relay the response message via WebSocket
      let responseData = { message: part.message, model: part.model,
                           done: part.done };
    ...
    if(err.name === 'AbortError') {
      console.log('handleQuery: previous query aborted');
    }
    else {
      console.log('handleQuery error:', err.message);
    }
  }
}
